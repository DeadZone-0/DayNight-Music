...
public interface PlaylistDao {
    ...
    @Query("SELECT EXISTS(SELECT 1 FROM playlist_song_cross_ref WHERE playlistId = :playlistId AND songId = :songId LIMIT 1)")
    boolean isSongInPlaylistSync(long playlistId, String songId);

    @Transaction
    @Query("SELECT * FROM playlists ORDER BY createdAt DESC")
    List<PlaylistWithSongs> getAllPlaylistsWithSongsSync();
}
